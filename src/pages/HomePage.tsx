...
export default function HomePage() {
  ...
  const locationState = location.state as { tab?: Tab; teamId?: string; measurementTab?: boolean } | null

  const savedState = (window.history.state?.homeState ?? {}) as { tab?: Tab; teamId?: string; personalSubTab?: PersonalSubTab; teamSubTab?: TeamSubTab }
  ...
  const initPersonalSubTab: PersonalSubTab = locationState?.measurementTab ? 'measurement' : savedState.personalSubTab ?? 'checklist'
  const initTeamSubTab: TeamSubTab = locationState?.measurementTab ? 'measurement' : savedState.teamSubTab ?? 'checklist'

  const [tab, setTab] = useState<Tab>(initTab)
  const [selectedTeamId, setSelectedTeamId] = useState<string>(initTeamId)
  const [personalSubTab, setPersonalSubTab] = useState<PersonalSubTab>(initPersonalSubTab)
  const [teamSubTab, setTeamSubTab] = useState<TeamSubTab>(initTeamSubTab)
  ...
  useEffect(() => {
    window.history.replaceState(
      { ...window.history.state, homeState: { tab, teamId: selectedTeamId, personalSubTab, teamSubTab } },
      ''
    )
  }, [tab, selectedTeamId, personalSubTab, teamSubTab])
  ...
  const handleCreateTeam = async () => {
    ...
    if (error?.code === '23505') { setTeamNameError('이미 사용 중인 팀 이름입니다.'); return }
  ...
  // 현재 서브탭이 체크리스트인지
  const isChecklistTab = tab === 'personal'
    ? personalSubTab === 'checklist'
    : teamSubTab === 'checklist'
    ...
    <div className="min-h-screen" style={{ background: T.bg }}>
        ...
        <div className="px-4 py-2 relative" style={{ background: T.surface, borderBottom: `1px solid ${T.border}` }}>
          <div className="flex items-center justify-between">
            <button onClick={() => setShowTeamDropdown(!showTeamDropdown)}
              className="flex items-center gap-2 text-sm font-medium" style={{ color: T.text }}>
              <span>{selectedTeam?.name ?? '팀을 선택하세요'}</span>
              <span style={{ color: T.muted }}>▾</span>
            </button>
            {selectedTeamId && (
              <button onClick={() => navigate(`/team/${selectedTeamId}/settings`)}
            ...
            <div className="absolute top-full left-0 right-0 z-10 shadow-2xl" style={{ background: T.surface, border: `1px solid ${T.border}` }}>
              {teams.map((team) => (
                <button key={team.id}
                  ...
                  className="w-full text-left px-4 py-3 text-sm transition-colors" style={{ color: T.text }}
              ...
              <div style={{ borderTop: `1px solid ${T.border}` }}>
                <button onClick={() => { navigate('/join-team'); setShowTeamDropdown(false) }}
                  className="w-full text-left px-4 py-3 text-sm" style={{ color: T.accent }}>
                  + 코드로 합류
                </button>
                <div className="px-4 py-2 flex gap-2">
                  <input value={newTeamName}
                    onChange={(e) => { setNewTeamName(e.target.value); setTeamNameError('') }}
                    placeholder="새 팀 이름" className="flex-1 rounded px-2 py-1 text-sm outline-none"
                ...
                {teamNameError && <p className="px-4 pb-2 text-xs" style={{ color: T.danger }}>{teamNameError}</p>}
              </div>
            </div>
          )}
        </div>
      ...
      {tab === 'personal' && (
        <div className="flex px-4 gap-4" style={{ background: T.bg, borderBottom: `1px solid ${T.border}` }}>
          {([['checklist', '체크리스트'], ['measurement', '측정']] as [PersonalSubTab, string][]).map(([key, label]) => (
            <button key={key} onClick={() => setPersonalSubTab(key)}
              className="py-2.5 text-sm font-medium transition-colors"
              style={{
                color: personalSubTab === key ? T.accent : T.muted,
                borderBottom: personalSubTab === key ? `2px solid ${T.accent}` : '2px solid transparent',
              }}>
              {label}
            </button>
          ))}
      ...
      {/* 콘텐츠 */}
      {tab === 'personal' && personalSubTab === 'measurement' ? (
        <MeasurementList ownerType="personal" ownerId={user?.id ?? ''} />
      ...
      {/* + 버튼 (체크리스트 탭에서만) */}
      {isChecklistTab && (tab === 'personal' || !!selectedTeamId) && (
...
function ChecklistCard({ checklist, onPress, onDelete }: { checklist: Checklist; onPress: () => void; onDelete: () => void }) {
  ...
  return (
    <>
      <div onClick={onPress} className="rounded-xl px-4 py-3 cursor-pointer transition-colors"
      ...
      {showConfirm && (
        <div className="fixed inset-0 flex items-center justify-center z-50 px-4"
          style={{ background: 'rgba(0,0,0,0.7)' }} onClick={cancelDelete}>
          <div className="rounded-2xl p-6 w-full max-w-xs text-center"
            ...
            <div className="flex gap-3">
              <button onClick={cancelDelete} className="flex-1 rounded-xl py-2.5 text-sm"
                style={{ border: `1px solid ${T.border}`, color: T.muted }}>취소</button>
              <button onClick={confirmDelete} className="flex-1 rounded-xl py-2.5 text-sm font-medium"
